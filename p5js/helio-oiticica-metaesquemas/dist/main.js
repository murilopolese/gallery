...
function setup() {
  ...
  let N = 7;
  let size_x = width/(N+1)
  let size_y = height/(N+1)
  for (let i = 0; i < N; i++) {
    for (let j = 0; j < N; j++) {
      let x = map(i, 0, N, size_x, width)
      let y = map(j, 0, N, size_y, height)

      // const rect = Bodies.polygon(
      //   x, y, 3, size_x*random(0.5, 0.65)
      // )
      ...
      rect.friction = 0.8
      rect.frictionAir = 0.8
      rect.restitution = 1
      Body.rotate(rect, random(-PI/64, PI/64))
      shapes.push( rect )
    }
  }
...
function draw() {
  ...
  // // sometimes gravity
  // if (frameCount % 60 < 20 + 15*sin(radians(frameCount*5))) {
  //   const f = 1
  //   engine.gravity.y = map(sin(radians(frameCount*8.0)), -1, 1, -f, f) * sin(radians(frameCount*0.2))
  //   engine.gravity.x = map(cos(radians(frameCount*5.9)), -1, 1, -f, f) * sin(radians(frameCount*0.1))
  // } else {
  //   engine.gravity.x = 0
  //   engine.gravity.y = 0
  // }
  ...
  for (let j in shapes) {
    ...
    push()
    fill(255, 0, 0)
    // text(distancia_x, body.position.x, body.position.y)
    pop()
